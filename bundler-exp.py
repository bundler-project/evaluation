...
copa = 'sudo ~/ccp_copa/target/release/copa --ipc=unix --delta_mode=NoTCP --default_delta=0.125 2> {}/copa.out'
algs = {
    'nimbus': nimbus,
    'bbr': bbr,
    'osc': osc,
...
def write_etg_config(name, args):
    with open(name, 'w') as f:
        ...
        f.write("req_size_dist ./MIXED_CDF\n")
...
def remote_script(args):
    ...
    if not args.do_fct_experiment:
        ...
        exc = 'cd ~/bundler/scripts && ./run-multiple-server.sh {}'.format(args.conns)
...
def local_script(args):
    outbox = 'sudo ~/bundler/box/target/release/outbox --filter "portrange 5000-6000" --iface ingress --inbox 10.1.1.2:28316 {} --sample_rate {} 2> {}/outbox.out'.format(
        "--no_ethernet",
        args.samplerate,
        args.outdir,
    )
