            ...
            if !aws.contains_key(&name) {
                ...
                let m = aws::Setup::default()
                    ...
                    .setup(move |vm| {
                        let name = nm.clone();
                        Box::pin(async move {
                            cloud::install_basic_packages(&vm.ssh)
                                .await
                                .wrap_err(eyre!("install_basic_packages failed for {:?}", name))?;
                            debug!("finished apt install");
                            cloud::get_tools(&vm.ssh)
                                .await
                                .wrap_err(eyre!("get_tools failed for {:?}", name))?;
                            Ok(())
                        })
                    });
                        ...
                        .setup(move |vm| {
                            ...
                            Box::pin(async move {
                                cloud::install_basic_packages(&vm.ssh)
                                    .await
                                    .wrap_err(eyre!(
                                        "install_basic_packages failed for {:?}",
                                        name
                                    ))?;
                                debug!("finished apt install");
                                cloud::get_tools(&vm.ssh)
